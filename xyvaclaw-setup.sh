...
if [ -d "$XYVACLAW_HOME" ]; then
    ...
    read -p "  是否备份并重新部署？(y=备份覆盖 / n=合并 / q=退出) " -n 1 -r
    echo ""
    if [[ $REPLY =~ ^[Yy]$ ]]; then
...
if [ ! -f "$WRAPPER_PATH" ] || [ ! -x "$WRAPPER_PATH" ]; then
    ...
    read -p "  需要 sudo 权限写入 /usr/local/bin/xyvaclaw，继续？(y/n) " -n 1 -r
    echo ""
    if [[ $REPLY =~ ^[Yy]$ ]]; then
...
echo ""
echo -e "  ${BOLD}启动方式:${NC}"
echo "    # 方式 1: 使用 xyvaclaw 命令"
echo "    xyvaclaw gateway"
echo ""
echo "    # 方式 2: 使用 openclaw 命令（需要先 export）"
echo "    export OPENCLAW_HOME=\$HOME/.xyvaclaw"
echo "    openclaw gateway"
echo ""
echo -e "  ${BOLD}常用命令:${NC}"
echo "    xyvaclaw gateway status    # 查看状态"
echo "    xyvaclaw agents list       # 查看 agent"
echo "    xyvaclaw cron list         # 查看定时任务"
echo ""
echo -e "  ${BOLD}首次启动注意:${NC}"
echo "    - 会下载本地 embedding 模型（约 70MB），请耐心等待"
echo "    - 飞书需在开放平台配置 webhook 回调地址"
echo ""
