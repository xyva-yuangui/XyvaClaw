...
export type SendFeishuMessageParams = {
  ...
  accountId?: string;
  /** Force message type when needed; defaults to auto routing. */
  forceMsgType?: "text" | "post";
};

type FeishuFormatDirective = "auto" | "card" | "post" | "text";

export function resolveFeishuFormatDirective(rawText: string): {
  directive: FeishuFormatDirective;
  text: string;
} {
  const text = rawText ?? "";
  const trimmed = text.trimStart();
  const explicit = trimmed.match(/^\[(?:format|render)\s*:\s*(card|post|text|auto)\]\s*/i);
  if (explicit) {
    return {
      directive: explicit[1].toLowerCase() as FeishuFormatDirective,
      text: trimmed.slice(explicit[0].length),
    };
  }
  return { directive: "auto", text };
}

export function shouldUseCardAuto(text: string): boolean {
...
function shouldUsePostAuto(text: string): boolean {
  if (!text || !text.trim()) return false;
  const lineCount = text.split(/\r?\n/).filter((line) => line.trim().length > 0).length;
  return lineCount >= 3 || /[•·]/.test(text) || /：\s*\n/.test(text);
}
...
function buildFeishuMessagePayload(params: {
  messageText: string;
  forceMsgType?: "text" | "post";
}): {
  content: string;
  ...
  const { messageText, forceMsgType } = params;
  if (forceMsgType === "text") {
    return buildFeishuTextMessagePayload({ messageText });
  }
  if (forceMsgType === "post") {
    return buildFeishuPostMessagePayload({ messageText });
  }
  if (shouldUsePostAuto(messageText)) {
    return buildFeishuPostMessagePayload({ messageText });
  }
  return buildFeishuTextMessagePayload({ messageText });
}
...
export async function sendMessageFeishu(
  ...
  const { cfg, to, text, replyToMessageId, replyInThread, mentions, accountId, forceMsgType } = params;
  ...
  const { content, msgType } = buildFeishuMessagePayload({ messageText, forceMsgType });
...
// Error codes indicating the message is already deleted/withdrawn — safe to ignore.
const DELETE_IDEMPOTENT_CODES = new Set([230011, 231003, 230001]);

/**
 * Delete (recall) a bot-sent message. Silently ignores already-deleted messages.
 * Used to clean up ephemeral status messages (ACK, watchdog reminders, queue notices)
 * after the final reply has been delivered.
 */
export async function deleteMessageFeishu(params: {
  ...
  try {
    ...
    // Idempotent: if already deleted/withdrawn, treat as success
    const code = (err as { code?: number }).code;
    if (typeof code === "number" && DELETE_IDEMPOTENT_CODES.has(code)) {
      return true;
    }
