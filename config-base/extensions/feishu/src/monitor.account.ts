...
import { prewarmFeishuStreamingToken } from "./streaming-card.js";
...
function registerEventHandlers(
  ...
  const inboundDebouncer = core.channel.debounce.createInboundDebouncer<FeishuMessageEvent>({
    ...
    buildKey: (event: FeishuMessageEvent) => {
    ...
    shouldDebounce: (event: FeishuMessageEvent) => {
    ...
    onFlush: async (entries: FeishuMessageEvent[]) => {
...
export async function monitorSingleAccount(params: MonitorSingleAccountParams): Promise<void> {
  ...
  if (account.config.streaming !== false && account.appId && account.appSecret) {
    void prewarmFeishuStreamingToken({
      appId: account.appId,
      appSecret: account.appSecret,
      domain: account.domain,
    }).catch((err) => {
      log(`feishu[${accountId}]: streaming token prewarm failed: ${String(err)}`);
    });
  }
