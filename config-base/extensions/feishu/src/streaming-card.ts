...
export async function prewarmFeishuStreamingToken(creds: Credentials): Promise<void> {
  const maxRetries = 3;
...
export class FeishuStreamingSession {
  ...
  private async updateCardContent(text: string, onError?: (error: unknown) => void): Promise<void> {
    ...
    await fetchWithSsrFGuard({
      ...
      .then(async ({ release }: { release: () => Promise<void> }) => {
        await release();
      })
  ...
  async update(text: string): Promise<void> {
    ...
    this.queue = this.queue.then(async () => {
      ...
      await this.updateCardContent(mergedText, (e: unknown) => this.log?.(`Update failed: ${String(e)}`));
    });
    await this.queue;
  }
