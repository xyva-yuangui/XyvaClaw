...
async function sendOutboundText(params: {
  ...
  const normalized = resolveFeishuFormatDirective(text);

  if (
    normalized.directive === "card" ||
    (normalized.directive !== "text" &&
      (renderMode === "card" || (renderMode === "auto" && shouldUseCardAuto(normalized.text))))
  ) {
  ...
  return sendMessageFeishu({
    ...
    replyToMessageId,
    forceMsgType:
      normalized.directive === "post" ? "post" : normalized.directive === "text" ? "text" : undefined,
  });
}
