...
export class LcmContextEngine implements ContextEngine {
  ...
  async afterTurn(params: {
    ...
    try {
      ...
      if (leafTrigger.shouldCompact) {
        this.compactLeafAsync({
          ...
          console.error(`[lcm] afterTurn leaf compaction failed: ${err instanceof Error ? err.message : String(err)}`);
        });
      }
    } catch (err) {
      console.error(`[lcm] afterTurn leaf trigger check failed: ${err instanceof Error ? err.message : String(err)}`);
    }
    ...
    try {
      ...
      console.error(`[lcm] afterTurn proactive compaction failed: ${err instanceof Error ? err.message : String(err)}`);
    }
  }
  ...
  async assemble(params: {
    ...
    try {
      ...
      // P4: Session length monitoring — warn when context is heavily utilized
      const SESSION_LENGTH_WARN_THRESHOLD = 500;
      const TOKEN_USAGE_WARN_RATIO = 0.75;
      const messageCount = await this.conversationStore.getMessageCount(conversation.conversationId);
      const tokenUsageRatio = tokenBudget > 0 ? assembled.estimatedTokens / tokenBudget : 0;
      let sessionLengthHint: string | undefined;
      if (messageCount >= SESSION_LENGTH_WARN_THRESHOLD || tokenUsageRatio >= TOKEN_USAGE_WARN_RATIO) {
        const pct = Math.round(tokenUsageRatio * 100);
        sessionLengthHint = `[系统提示] 当前对话已累积${messageCount}条消息，上下文使用率${pct}%。建议适时开启新对话以保持响应速度。`;
        console.error(`[lcm] session length warning: session=${params.sessionId}, messages=${messageCount}, tokenUsage=${pct}%`);
      }

      const systemPromptParts: string[] = [];
      if (assembled.systemPromptAddition) {
        systemPromptParts.push(assembled.systemPromptAddition);
      }
      if (sessionLengthHint) {
        systemPromptParts.push(sessionLengthHint);
      }
      ...
      console.error(`[lcm] assemble failed:`, err instanceof Error ? err.message : err);
      return {
        messages: params.messages,
        estimatedTokens: 0,
      };
