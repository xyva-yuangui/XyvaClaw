...
export class CompactionEngine {
  ...
  private async summarizeWithEscalation(params: {
    ...
    if (estimateTokens(summaryText) >= inputTokens) {
      ...
      if (estimateTokens(summaryText) >= inputTokens) {
        // Extract key sentences (first, last, and any containing important markers)
        const sentences = sourceText.split(/(?<=[。！？\n])/);
        const keyParts: string[] = [];
        const importantMarkers = /(?:决定|结论|结果|重要|关键|注意|TODO|FIXME|错误|失败|成功|完成)/;
        if (sentences.length > 0) keyParts.push(sentences[0].trim());
        for (const s of sentences.slice(1, -1)) {
          if (importantMarkers.test(s)) keyParts.push(s.trim());
        }
        if (sentences.length > 1) keyParts.push(sentences[sentences.length - 1].trim());
        const extracted = keyParts.join("\n").slice(0, FALLBACK_MAX_CHARS);
        const fallbackContent = extracted.length > 0 ? extracted : sourceText.slice(0, FALLBACK_MAX_CHARS);
        summaryText = `${fallbackContent}\n[Fallback summary from ${inputTokens} tokens — key sentences extracted]`;
        level = "fallback";
        console.error(`[lcm] compaction fallback triggered: inputTokens=${inputTokens}, extractedLen=${fallbackContent.length}`);
      }
    }
