...
const BROAD_TIME_RANGE_PATTERNS = [
  ...
  // Chinese broad time range patterns
  /(过去|最近|近|上|前)(一|两|三|四|五|六|七|八|九|十|[\d]+)[个]?(月|季度|年|周)/,
  /(从|自|始于).{0,20}(到|至|截止|直到)/,
  /(时间线|历史|长期|趋势分析|变化过程|演变)/,
  /(这[几半]年|今年以来|去年|前年|上半年|下半年)/,
];
...
const MULTI_HOP_QUERY_PATTERNS = [
  ...
  // Chinese multi-hop / deep reasoning patterns
  /(根本原因|因果链|事件链|因果关系|深层原因)/,
  /(为什么会?导致|怎么引[起发]的|如何演变)/,
  /(综合分析|系统分析|全面分析|深度分析|关联分析)/,
  /(对比.{0,10}(和|与|跟).{0,10}(区别|差异|不同|优劣))/,
];
...
export function decideLcmExpansionRouting(
  ...
  return {
    ...
    indicators: {
      broadTimeRange,
      multiHopRetrieval,
      suggestReasoning: shouldDelegate || multiHopRetrieval || (broadTimeRange && tokenRisk.level !== "low"),
    },
    triggers: {
      directByNoCandidates,
