  ...
  return {
    ...
    complete: async ({
      ...
      try {
        ...
        const LCM_COMPLETE_TIMEOUT_MS = 60_000;
        const completePromise = mod.completeSimple(
          resolvedModel,
          {
        ...
        const timeoutPromise = new Promise<never>((_, reject) =>
          setTimeout(() => reject(new Error(`LCM completeSimple timeout after ${LCM_COMPLETE_TIMEOUT_MS}ms (provider=${providerId}, model=${modelId})`)), LCM_COMPLETE_TIMEOUT_MS),
        );
