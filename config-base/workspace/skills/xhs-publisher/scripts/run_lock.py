...
# Lock timeout in seconds (auto-cleanup if process hangs)
...
def _cleanup_stale_lock(path: str) -> tuple[bool, dict[str, Any]]:
    ...
    # Auto-cleanup if lock is too old (timeout protection)
    if isinstance(started_at, str):
        try:
            start_time = datetime.fromisoformat(started_at.replace("Z", "+00:00"))
            age = datetime.now(timezone.utc) - start_time
            if age.total_seconds() > LOCK_TIMEOUT_SECONDS:
                # Lock has expired, cleanup even if PID might exist
                try:
                    os.remove(path)
            ...
            pass  # Invalid timestamp, fall through to normal cleanup
    ...
    if isinstance(pid, int):
        ...
        if isinstance(started_at, str) and started_at:
            ...
            try:
                ...
                if age_minutes > 0:
                    msg += f" ({age_minutes}m ago)"
                if age.total_seconds() > LOCK_TIMEOUT_SECONDS:
                    msg += f" [LOCK EXPIRED - auto-cleanup in {LOCK_TIMEOUT_SECONDS}s]"
            except (ValueError, TypeError):
                pass
