...
SELECTORS = {
    ...
    # Publish button - "发布笔记" is the main publish button
...
UPLOAD_WAIT = 15  # seconds for image upload processing (reduced from 60 to avoid WebSocket timeout)
...
ACTION_INTERVAL = 5  # seconds between actions
...
class XiaohongshuPublisher:
    ...
    def _close_tab(self):
        """Close the current CDP tab."""
        try:
            print("[cdp_publish] Closing browser tab...")
            # Get current target ID and close it
            targets = self._send("Target.getTargets", {})
            for target in targets.get("targetInfos", []):
                if target.get("type") == "page":
                    self._send("Target.closeTarget", {"targetId": target["targetId"]})
                    break
            self._sleep(2, minimum_seconds=1.0)
            print("[cdp_publish] Browser tab closed.")
        except Exception as e:
            print(f"[cdp_publish] Warning: Failed to close tab: {type(e).__name__}")
    ...
    def _upload_images(self, image_paths: list[str]):
        ...
        # Poll for editor to appear (keep WebSocket active)
        print("[cdp_publish] Images uploaded. Waiting for editor...")
        editor_ready = False
        for i in range(UPLOAD_WAIT):  # Check every second
            self._sleep(1, minimum_seconds=0.5)
            try:
                ready = self._evaluate(f"""
                    !!(document.querySelector('{SELECTORS["content_editor"]}') ||
                       document.querySelector('{SELECTORS["content_editor_alt"]}'))
                """)
                if ready:
                    print(f"[cdp_publish] Editor ready after {i+1}s")
                    editor_ready = True
                    break
            except Exception:
                pass  # Keep waiting
        ...
        if not editor_ready:
            print(f"[cdp_publish] Editor not ready after {UPLOAD_WAIT}s, continuing anyway...")
    ...
    def _fill_title(self, title: str):
        """Fill in the article title using native setter (bypasses React/TapTap interception)."""
        ...
        # Retry logic (5 attempts, 2s each = 10s max)
            ...
            for selector in selectors:
                try:
                    ...
                    if found:
                        ...
                        # Use native setter to bypass React/TapTap interception
                        self._evaluate(f"""
                            (function() {{
                                ...
                                // Focus first to ensure page is ready
                                el.focus();
                        ...
                        """)
                        print(f"[cdp_publish] Title set successfully with selector: {selector}")
                        self._sleep(1.5, minimum_seconds=0.5)
                        return
                except Exception:
                    pass  # Continue to next selector
            
            print(f"[cdp_publish] Waiting for title input (attempt {attempt+1}/5)...")
            self._sleep(2.0, minimum_seconds=1.0)
        
        # Debug: try to find any input element
        try:
            inputs = self._evaluate("""
                (function() {
            ...
            """)
            print(f"[cdp_publish] Debug: Found {len(inputs)} input elements:")
            for i, inp in enumerate(inputs[:5]):
                print(f"  [{i}] type={inp.get('type')}, placeholder={inp.get('placeholder', '')[:30]}, class={inp.get('class', '')[:30]}")
        except Exception as e:
            print(f"[cdp_publish] Debug failed: {e}")
        
        raise CDPError("Could not find title input element after 5 attempts. Page structure may have changed.")

    def _fill_content(self, content: str):
        """Fill in the article body content using TipTap/ProseMirror editor."""
        print(f"[cdp_publish] Setting content ({len(content)} chars)...")
        self._sleep(ACTION_INTERVAL, minimum_seconds=0.25)

        # Priority selectors for content editor
        selectors = [
            ...
            if found:
                ...
                self._evaluate(f"""
                    (function() {{
                        ...
                        // Dispatch input event to trigger editor update
                        el.dispatchEvent(new Event('input', {{ bubbles: true }}));
                    }})();
                """)
                print(f"[cdp_publish] Content set successfully with selector: {selector}")
                return

        raise CDPError("Could not find content editor element. Page structure may have changed.")
    ...
    def _click_publish(self):
        """Click the publish button ('发布笔记') and wait for URL change."""
        print("[cdp_publish] Clicking publish button...")
        self._sleep(ACTION_INTERVAL, minimum_seconds=0.5)

        # Try both button texts
        for btn_text in [SELECTORS["publish_button_text"], SELECTORS["publish_button_text_alt"]]:
            clicked = self._evaluate(f"""
                (function() {{
                    console.log('[cdp_publish] Searching for: {btn_text}');
                    
                    // Search all buttons
                    var buttons = document.querySelectorAll('button, .ant-btn, [role="button"], [class*="btn"]');
                    for (var i = 0; i < buttons.length; i++) {{
                        var t = buttons[i].textContent.trim();
                        if (t === '{btn_text}') {{
                            console.log('[cdp_publish] Clicking: ' + t);
                            buttons[i].click();
                            return true;
                        }}
                    }}
            ...
            if clicked:
                print(f"[cdp_publish] Publish button clicked ('{btn_text}').")
                break
        
        if not clicked:
            raise CDPError("Could not find publish button")
        
        # Wait for URL change (up to 30 seconds)
        print("[cdp_publish] Waiting for publish...")
        for i in range(6):  # 6 * 5 = 30 seconds
            self._sleep(5, minimum_seconds=4.0)
            try:
                url = self._evaluate("window.location.href")
                print(f"[cdp_publish] Check {i+1}/6: {url[:80]}...")
                
                if 'published=true' in url or '/explore/' in url or '/user/profile' in url:
                    print(f"[cdp_publish] ✅ Published! URL: {url}")
                    return url
            except Exception as e:
                print(f"[cdp_publish] Connection closed (redirect): {e}")
                return "redirected"
        
        print("[cdp_publish] ❌ URL unchanged after 30s")
        return None
    ...
    def publish(
        ...
        4. Fill title
        5. Fill content
        6. Select cover image (auto: first image)
        7. Select category (auto: default)
...
def main():
    ...
    try:
        if args.command == "check-login":
            ...
            # Set timeout handler for publish operation (120 seconds)
            PUBLISH_TIMEOUT = 600  # seconds (increased from 120)
            
            def timeout_handler(signum, frame):
                print(f"\n[cdp_publish] ERROR: Publish operation timed out after {PUBLISH_TIMEOUT}s")
                print("[cdp_publish] Cleaning up...")
                try:
            ...
            if args.command == "publish":
                # Only set timeout for actual publish, not fill
                if hasattr(signal, 'SIGALRM'):
                    signal.signal(signal.SIGALRM, timeout_handler)
                    signal.alarm(PUBLISH_TIMEOUT)
            ...
            try:
                ...
                if args.command == "publish":
                    ...
                    # Aligned with ship-to-redbook-skill: no URL verification
