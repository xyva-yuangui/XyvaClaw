...
def main():
    ...
    try:
        ...
        # Skip topic selection - causes publish to fail
    ...
    if should_publish:
        ...
        try:
            ...
            # Wait for URL change (up to 60 seconds)
            print("[pipeline] Waiting for publish confirmation...")
            publish_confirmed = False
            for i in range(12):  # 12 * 5 = 60 seconds
                publisher._sleep(5, minimum_seconds=4.0)
                try:
                    current_url = publisher._evaluate("window.location.href")
                    if current_url and ('published=true' in current_url or '/explore/' in current_url or '/user/profile' in current_url):
                        print(f"[pipeline] Publish confirmed! URL: {current_url}")
                        publish_confirmed = True
                        break
                except Exception:
                    # Connection may be closed due to page redirect
                    print(f"[pipeline] Page redirected (publish likely successful)")
                    publish_confirmed = True
                    break
            ...
            if publish_confirmed:
                ...
                if note_link:
                    print(f"[pipeline] Note URL: {note_link}")
            else:
                print("PUBLISH_STATUS: FAILED ❌ (URL did not change)")
                print("[pipeline] Please check manually in browser")
        except CDPError as e:
